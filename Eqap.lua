...
io.write('\n\27[1;35mSend UserName For Sudo : ارسل معرف Owner 🎖 ...\n\27[0;39;49m')
...
io.write('\n\27[1;31mThe UserName was not Saved : لم يتم حفظ معرف Owner 🎖\n\27[0;39;49m')
end 
os.execute('lua Eqap.lua')
end
...
if text == 'الاوامر الخدميه' or text == 'العوده' then  
local Text_keyboard = 'هذي الاوامر الخدميه الخاصة بلسورس'
local List_keyboard = {
{'احسب عمرك'},
{'الابراج','اقتباس'}
{'•┉ • ┉ • ┉ • ┉ • ┉•'},
{'افتارات عيال'},
{'افتارات انمي','افتارات بنات'},
{'غنيلي','ضحكني'},
{'قناة السورس','البوت السحري'},
{'•┉ • ┉ • ┉ • ┉ • ┉•'},
{'فك التشفير','تشفير'},
{'بوت الباحث القراني','خلفية عشوائيه'},
{'رجوع ،‘'},
}
...
if text == ("رفع كيكه") and tonumber(msg.reply_to_message_id_) ~= 0
if not redis:get(bot_id..'Eqap:Cheking:Seted'..msg.chat_id_) and not Owner(msg) then
send(msg.chat_id_, msg.id_,' تستطيع رفع احد وذالك لكن الآن تم تعطيل الرفع من قبل المنشئين')
return false
end
function FunctionStatus(arg, result)
redis:sadd(bot_id.."Eqap:Mempar:Group"..msg.chat_id_, result.sender_user_id_)
Send_Options(msg,result.sender_user_id_,"reply","تم رفعه إلى قائمة الكيكه")  
end
tdcli_function ({ID = "GetMessage",chat_id_ = msg.chat_id_,message_id_ = tonumber(msg.reply_to_message_id_)}, FunctionStatus, nil)
end

if text == ("تنزيل كيكه") and tonumber(msg.reply_to_message_id_) ~= 0
function FunctionStatus(arg, result)
redis:srem(bot_id.."Eqap:Mempar:Group"..msg.chat_id_, result.sender_user_id_)
Send_Options(msg,result.sender_user_id_,"reply","تم تنزيله من قائمة الكيكه")  
end
tdcli_function ({ID = "GetMessage",chat_id_ = msg.chat_id_,message_id_ = tonumber(msg.reply_to_message_id_)}, FunctionStatus, nil)
end
...
if text and text:match("^رفع كيكه @(.*)$")
if not redis:get(bot_id..'Eqap:Cheking:Seted'..msg.chat_id_) and not Owner(msg) then
send(msg.chat_id_, msg.id_,' تستطيع رفع احد وذالك لكن الآن تم تعطيل الرفع من قبل المنشئين')
return false
end
...
redis:sadd(bot_id.."Eqap:Mempar:Group"..msg.chat_id_, result.id_)
Send_Options(msg,result.id_,"reply","تم رفعه إلى قائمة الكيكه")  
else
send(msg.chat_id_, msg.id_,"المعرف غلط ")
end
end
tdcli_function ({ID = "SearchPublicChat",username_ = text:match("^رفع كيكه @(.*)$")}, FunctionStatus, nil)
end
...
if text == ("قائمة الكيكه") (msg)
local list = redis:smembers(bot_id.."Eqap:Mempar:Group"..msg.chat_id_)
if #list == 0 then
return send(msg.chat_id_, msg.id_, "⇽ لا يوجد")
end
Mempar = "\n⇽ قائمة الكيكه في المجموعه \n•———————•\n"
for k,v in pairs(list) do
tdcli_function ({ID = "GetUser",user_id_ = v},function(arg,data) 
if data.username_ then
username = '[@'..data.username_..']'
...
Mempar = Mempar..""..k.."~ : "..username.."\n"
if #list == k then
return send(msg.chat_id_, msg.id_, Mempar)
end
end,nil)
end
end
...
if text == ("مسح قائمة الكيكه") and Admin(msg) then
redis:del(bot_id.."Eqap:Mempar:Group"..msg.chat_id_)
send(msg.chat_id_, msg.id_, "⇽  تم مسح قائمة الكيكه في المجموعه")
end
...
local thekkk = {"انا مو بس ابي اغير جو انا ابي اغير ارض.","إذا جاء يوم ولم تبتسم لك الحياة أجي انا لحد باب بيتكم وابتسم لك وش تبي بعد يا دلبي .","لا تخسر الشخص اللي يخرب عليك هيبة عصبيتك ويخليك تضحك .","هدوئي الجميل وحضوري المتقطع نوع من انواع الفخامة","🧔🏻: ليش متغيره وصايره ماتكلميني نفس قبل ؟
‏👩🏻: بالغلط جاملتك و توهقت","ماذا لو عاد معتذراً ؟ 
‏👩🏻: أي واحد فيهم","كل البجاحات تهون الا بجاحة الي يغلط و يزعل.","‏الحب قدر ! ، والغيرة صحن وانتم ملاعق والعشا عندنا .
 الاخيره للقراءة فقط .","هذه الأيام ‏لا النفسيه تساعد ولأ الناس ينبلعون .","‏واحد يقول امي شرت لأخوي الصغير  سيكل 🚲 علشانه دب وتبيه ينحف.
وصار كل يوم يروح عليه يشتري بروست 🍗 ويرجع.😭😭","‏سأروي لكم قصة حياتي بإختصار 
الناس في وادي وأنا في وادي.","‏بمناسبة هذا الهدوء والطفش وش جمع كلمة كاتشب ؟ .","تدرون وش أفكر فيه الحين إذا مت من بيعلمكم إني مت ؟ .","‏انا إنسانه  ما احب ادخل علاقات 
احب ادخل مطاعم بس","لازم احب عشان اذا وريتها صوري تقول يمممه خقيت .","تحية وَ تقدير لــكل بنت تزوجت و ما ازعجتنا بـ صور اصابعها واصابع زوجها .","الدنيا صغيره يمكن انتي اللي تضحكين ع بيسياتي تصيرين ام عيالي .","ترا لما تبلع ريقك انت كذا جالس تتفل على نفسك من داخل .","تغيرت لدرجه صرت اشوف عصير في الثلاجه وماشربه .","لكل شخص يعرفني والله انك محظوظ لاتنكر .","‏أصدق إعلان شفته في حياتي أنت مو أنت وأنت جيعان 😭😭.","قبل شوي مزحت مع صاحبي ب رشاش وكان يبي يموت اهخخخ وهذي اخر مرا اخاوي دلوعين .","الحين انتم ياللي ترسلون منشورات حب
‏منشوراتكم جالسه تجيب نتيجة ولا 
‏بس مزعجينا عالفاضي؟","اسوأ شعور انك تواسي واحد يصيح وصيحته  تضحك ، وقتها ما تدري تضحك والا تواسيه .","أخشى عليك من الوحدة في قلبي لذلك أدخلت معك جميع المزز .","وفي غيابها ؟ ، شبك غيرها يعني متوقعين ينتضرها ! .","اذا وصلت لمرحلة ان الاكل مايونسك ف انت صدق مكتئب.","لا تقول "وأخيراً افتكيت" لأنك طول ما انت عايش ما راح تفتك","‏شكلي لما كنت اغني لفي بينا يادنيا دوخينا يادنيا كانت ساعة استجابه.","‏نظامي مع الروابط اذا مافتح بخمس ثواني خلاص شكراً مايحتاج اشوفه."}
if Text== "غني" or Text=="غنيلي" then 
return sendMsg(msg.chat_id_,msg.id_,thekkk[math.random(#thekkk)])
if text == 'مقالات' then
